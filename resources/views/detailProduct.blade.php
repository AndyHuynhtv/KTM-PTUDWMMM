        ...
        <div class="product-show-detail">
            <div class="container">
                ...
                <div class="row product-detail">
                    <div class="col-md-6">
                        <img src="{{ asset($product->thumbnail) }}" alt="{{ $product->title }}" class="product-image">
                    </div>
                    <div class="col-md-6">
                        <h3>Chi tiết sản phẩm</h3>
                        <p><strong>Giá:</strong> {{ number_format($product->price) }}đ</p>
                        @if($product->discount)
                            <p><strong>Giá giảm:</strong> {{ number_format($product->discount) }}đ</p>
                        @endif
...
<script>
  document.getElementById('addToCartBtn').addEventListener('click', function() {
      ...
      let productThumbnail = "{{ asset($product->thumbnail) }}";
      ...
      // Thực hiện các hành động thêm sản phẩm vào giỏ hàng tại đây (sử dụng localStorage, Vuex, hoặc các công nghệ lưu trữ khác)
